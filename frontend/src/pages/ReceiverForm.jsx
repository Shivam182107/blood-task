...
const ReceiverForm = () => {
 ...
 const handleSearch = () => {
  try {
    const cityInput = ReceiverFormData.city.trim().toLowerCase();
    const bloodInput = ReceiverFormData.bloodGroup.trim().toLowerCase();

    const filtered = DonorsData.filter(
      (val) =>
        val.city.trim().toLowerCase() === cityInput &&
        val.bloodgroup.trim().toLowerCase() === bloodInput
    );
      ...
      <div className="bg-[#111] border border-[#2a2a2a] rounded-2xl w-full max-w-lg mx-auto p-6 sm:p-8">
        <div className="mb-6">
          ...
          <p className="text-[#666] text-sm mt-1">
            Fill in your details to search nearby donors
          </p>
        </div>

        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-1.5">
            <label className="text-[#888] text-[11px] font-medium uppercase tracking-wider">
              Full Name
            </label>
            <input
              type="text"
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <input
              type="text"
              name="bloodGroup"
              value={ReceiverFormData.bloodGroup}
              onChange={handleChange}
              placeholder="e.g. B+"
              className="bg-[#1a1a1a] border border-[#2a2a2a] focus:border-red-600 rounded-lg text-white text-sm px-3 py-2.5 outline-none placeholder-[#444] transition-colors w-full"
            />
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <input
              type="text"
              name="country"
              value={ReceiverFormData.country}
              onChange={handleChange}
              placeholder="India"
              className="bg-[#1a1a1a] border border-[#2a2a2a] focus:border-red-600 rounded-lg text-white text-sm px-3 py-2.5 outline-none placeholder-[#444] transition-colors w-full"
            />
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <textarea
              name="address"
              value={ReceiverFormData.address}
              onChange={handleChange}
              ...
              className={`flex items-center justify-center gap-2 border rounded-lg text-sm py-2.5 px-4 transition-all w-full
                ${
                  locationStatus === "success"
                    ? "bg-[#0d1f0e] border-green-800 text-green-400"
                    : locationStatus === "error"
                      ? "bg-[#1f0d0d] border-red-900 text-red-400"
              ...
              {locationStatus === "loading" ? (
                ...
                >
                  <path
                    ...
                    strokeWidth={1.5}
                    d="M12 8a4 4 0 100 8 4 4 0 000-8zm0 0V3m0 17v-2M3 12H5m14 0h2"
                  />
                </svg>
              )}
              <span>
